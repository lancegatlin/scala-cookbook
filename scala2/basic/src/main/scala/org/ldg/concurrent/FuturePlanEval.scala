...
object FuturePlanEval {
    ...
    FuturePlanCancelableEval(
      future, { () =>
        ...
        future.transformWith { _ =>
          Future.successful( () )
        } // ignore exceptions here, only completion matters
      }
    )
  }
  ...
  private def evalStep[A](
      ...
      implicit
      executionContext: ExecutionContext ): Future[( EvalState, Either[Throwable, A] )] = {
      ...
      case HandleErrorWith( prevStep, f ) =>
        evalPrevStep( state0, prevStep )
          .flatMap {
            ...
            // scalastyle:on lowercase.pattern.match
          }
  ...
  private def evalSleep(
      ...
      implicit
      executionContext: ExecutionContext ): Future[( EvalState, Either[Throwable, Unit] )] =
