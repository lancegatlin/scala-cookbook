...
object RetryCore {
  ...
  /**
    ...
    * @param onAfterFailure event handler for after each failure (which can modify retry state). None indicates don't
    *                       retry
      ...
      onBeforeAttempt( state0 )
        .flatMap { state1 =>
          attempt( state1 ).flatMap {
            ...
            case fatal @ Left( cause ) if !NonFatal( recurseUntilUltimateCause( cause ) ) =>
            ...
            case Left( cause ) =>
              onAfterFailure( state1, cause ).flatMap {
                ...
                // no delay indicates don't retry, return last state and error to caller
