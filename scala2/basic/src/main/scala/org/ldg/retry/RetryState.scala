...
case class RetryState(
  ...
  def onAfterFailure[F[_]:Monad](
    ...
    now: Instant
  ): F[(RetryState, Option[FiniteDuration])] = {
    ...
    def onAfterFailure(
      ...
      val maybeRetryDelay = retryAction match {
        case AttemptAgain(retryDelay) => Some(retryDelay)
        case _ => None
      }
    ...
    if (maybeMaxElapsedTime.forall(this.elapsed < _)) {
      retryConfig.shouldRetry(ultimateCause).flatMap { shouldRetry =>
        if (shouldRetry) {
      ...
      onAfterFailure(cause, StopRetry(ExceededMaxElapsedTime))
    }
  }
