...
class RetryPackageSpec extends AnyFlatSpec with Matchers {
  implicit val defaultRetryConfig: RetryConfig[IO] = RetryConfig
    .default[IO]().copy(
      calcRetryDelay = { ( _, _ ) =>
        Applicative[IO].pure( 0.millis )
      }
    )

  "retry with maxAttempts" should "retry the specified number of times" in {
    val attempts = new AtomicInteger( 0 )
    val retryPlan = RetryPlan[IO]( 3 )
  ...
  "retry with maxElapsedDuration" should "stop retrying after the specified time" in {
    val attempts = new AtomicInteger( 0 )
    val retryPlan = RetryPlan[IO]( 100.millis, 10.millis )
  ...
  "retry with maxAttempts and maxElapsedDuration" should "respect both constraints" in {
    val attempts = new AtomicInteger( 0 )
    val retryPlan = RetryPlan[IO]( 3, Some( 1.second ) )
  ...
  "RetryFunctionExt" should "wrap a function with retry logic" in {
    ...
    val function: String => IO[String] = input =>
      IO {
        if (attempts.incrementAndGet() < 3) throw new RuntimeException( "Failure" )
        s"Hello, $input"
      }
  ...
  "RetryFunction2Ext" should "wrap a two-argument function with retry logic" in {
    ...
    val function: ( String, Int ) => IO[String] = ( input, number ) =>
      IO {
        if (attempts.incrementAndGet() < 3) throw new RuntimeException( "Failure" )
        s"$input, $number"
      }
  ...
  "RetryFunction3Ext" should "wrap a three-argument function with retry logic" in {
    ...
    val function: ( String, Int, Boolean ) => IO[String] = ( input, number, flag ) =>
      IO {
        if (attempts.incrementAndGet() < 3) throw new RuntimeException( "Failure" )
        s"$input, $number, $flag"
      }
  ...
  "RetryFunction4Ext" should "wrap a four-argument function with retry logic" in {
    ...
    val function: ( String, Int, Boolean, Double ) => IO[String] = ( input, number, flag, value ) =>
      IO {
        if (attempts.incrementAndGet() < 3) throw new RuntimeException( "Failure" )
  ...
  "RetryFunction5Ext" should "wrap a five-argument function with retry logic" in {
    ...
    val function: ( String, Int, Boolean, Double, Char ) => IO[String] = ( input, number, flag, value, char ) =>
      IO {
        if (attempts.incrementAndGet() < 3) throw new RuntimeException( "Failure" )
  ...
  "RetryFunction6Ext" should "wrap a six-argument function with retry logic" in {
    ...
    val function: ( String, Int, Boolean, Double, Char, Long ) => IO[String] = ( input, number, flag, value, char, long ) =>
      IO {
        if (attempts.incrementAndGet() < 3) throw new RuntimeException( "Failure" )
  ...
  "RetryFunction7Ext" should "wrap a seven-argument function with retry logic" in {
    ...
    val function: ( String, Int, Boolean, Double, Char, Long, Float ) => IO[String] = ( input, number, flag, value, char, long, float ) =>
      IO {
        if (attempts.incrementAndGet() < 3) throw new RuntimeException( "Failure" )
  ...
  "RetryFunction8Ext" should "wrap an eight-argument function with retry logic" in {
    ...
    val function: ( String, Int, Boolean, Double, Char, Long, Float, Short ) => IO[String] = ( input, number, flag, value, char, long, float, short ) =>
      IO {
        if (attempts.incrementAndGet() < 3) throw new RuntimeException( "Failure" )
  ...
  "RetryFunction9Ext" should "wrap a nine-argument function with retry logic" in {
    ...
    val function: ( String, Int, Boolean, Double, Char, Long, Float, Short, Byte ) => IO[String] =
      ( input, number, flag, value, char, long, float, short, byte ) =>
        IO {
          if (attempts.incrementAndGet() < 3) throw new RuntimeException( "Failure" )
