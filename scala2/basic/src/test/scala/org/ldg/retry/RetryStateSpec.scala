...
class RetryStateSpec extends AnyFlatSpec with Matchers with Inside {
  class Fixture {
    ...
    implicit val retryConfig: RetryConfig[Id] = RetryConfig
      .default[Id]()
      .copy(
        onRetryEvent = { event =>
          eventsBuffer += event
  ...
  "initialState" should "initialize to the initial retry state" in new Fixture {
    ...
    initialState.maybeMaxElapsedDuration shouldBe maybeMaxElapsedTime
  ...
  it should "return None/ExceededMaxElapsedDuration when max elapsed time is exceeded" in new Fixture {
    ...
    val expectedRetryEvent1 = RetryEvent.OnAfterFailure(
      state1,
      retryableRuntimeException,
      RetryAction.StopRetry( FailedRetryReason.ExceededMaxElapsedDuration )
    )
    state2.maybeLastRetryEvent shouldBe Some( expectedRetryEvent1 )
  ...
  it should "return None/ExceededMaxElapsedDuration when retry delay is after max elapsed duration" in new Fixture {
    val longRetryDelayRetryConfig = retryConfig.copy(
      calcRetryDelay = { ( _, _ ) =>
        Applicative[Id].pure( 10.seconds )
      }
    )
    val state0 = RetryState.initial( 3, Some( 10.seconds ), correlationId )
    val now1 = now.plusSeconds( 2 )
    val state1 = state0.onBeforeAttempt[Id]( implicitly, now1 )
    state1.elapsed( now1 ) shouldBe 2.seconds
    val ( state2, None ) = state1.onAfterFailure[Id]( retryableRuntimeException )( implicitly, longRetryDelayRetryConfig, now.plusSeconds( 3 ) )
    val expectedRetryEvent1 = RetryEvent.OnAfterFailure(
      state1,
      retryableRuntimeException,
      RetryAction.StopRetry( FailedRetryReason.ExceededMaxElapsedDuration )
